...
// Default Partial Grunt reporter
defaultPartialReporter = function( result, options ) {
  var wrapData = options.wrapData;
  var out = result.map(function( message ) {
    var output = chalk.cyan( wrapData.cache[ message.file ] ) + ' ';
    output += chalk.red( '[' ) + chalk.yellow( 'L' + (message.lastLine - wrapData.lineOffset) ) +
      chalk.red( ':' ) + chalk.yellow( 'C' + message.lastColumn ) + chalk.red( '] ' );
    output += message.message;
    return output;
  });
...
selectReporter = function( options ) {
  ...
  if ( options.reporter ) {
    ...
    if ( options.wrapData ) {
      reporter = defaultPartialReporter;
    } else {
